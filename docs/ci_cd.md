...
- **Android debug build.** The frontend is built, synchronized with Capacitor,
  and compiled with `./gradlew assembleDebug` from `frontend/android`. On a
  push to `main`, the generated debug APK is published as the
  `sterna-debug-apk` GitHub Actions artifact; pull requests only validate the
  build.
